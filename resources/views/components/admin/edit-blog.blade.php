   ...
   <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
       <!-- Modal body -->
       <form action="/dashboard/{{ $blog->slug }}/update" method="post" id="blog-form">
           @csrf
           @method('PATCH')
           <div class="mb-4">
           ...
           <div class="mb-4">
               ...
               <select id="category" name="category_id"
                   ...
                   @foreach ($categories as $category)
                       <option value="{{ $category->id }}" @selected((old('category_id') ?? $blog->category->id) == $category->id)>{{ $category->name }}</option>
                   @endforeach
               </select>
               @error('category_id')
                   <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
           ...
           <div class="sm:col-span-2 mb-4">
               ...
               <textarea id="article" rows="4" name="article"
                   ...
                   placeholder="Write article here">{{ old('article', $blog->article) }}</textarea>
               <div id="editor">{!! old('article', $blog->article) !!}</div>
               @error('article')
                   <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
               @enderror
           </div>
