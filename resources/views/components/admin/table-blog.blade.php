                        ...
                        @foreach ($blogs as $blog)
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    {{ ($blogs->currentpage() - 1) * $blogs->perpage() + $loop->index + 1 }}</th>
                                <td class="px-4 py-3">{{ $blog->title }}</td>
                                <td class="px-4 py-3">{{ $blog->category->name }}</td>
                                <td class="px-4 py-3">{{ $blog->created_at->diffForHumans() }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button id="blog-{{ $blog->id }}-dropdown-button"
                                        data-dropdown-toggle="blog-{{ $blog->id }}-dropdown"
                                    ...
                                    <div id="blog-{{ $blog->id }}-dropdown"
                                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm"
                                            aria-labelledby="blog-{{ $blog->id }}-dropdown-button">
                                            <li>
                                                <a href="/dashboard/{{ $blog->slug }}"
                                            ...
                                            <li>
                                                <a href="/dashboard/{{ $blog->slug }}/edit"
                                                    class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg"
                                                        viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                        <path
                                            ...
                                            <li>
                                                <button type="button"
                                                    data-modal-target="deleteModal-{{ $blog->id }}"
                                    ...
                                    <!-- Delete modal -->
                                    <div id="deleteModal-{{ $blog->id }}" tabindex="-1" aria-hidden="true"
                                        ...
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            ...
                                            <div
                                                ...
                                                <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want
                                                    to delete article {{ $blog->title }}?</p>
                                                <div class="flex justify-center items-center space-x-4">
                                                    <button data-modal-toggle="deleteModal-{{ $blog->id }}"
                                                        type="button"
                                                        class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No,
                                                        cancel</button>
                                                    <form action="/dashboard/{{ $blog->slug }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
