  <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
      <article
              ...
              <address class="flex items-center mb-6 not-italic">
                  <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      <img class="mr-4 w-16 h-16 rounded-full"
                          src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : asset('img/avatar1.jpg') }}"
                          alt="{{ Auth::user()->name }}">
                      <div>
                          <a href="#" rel="author"
          ...
          <div>
              <a href="/dashboard/{{ $blogs->slug }}/edit"
              ...
              <button type="button" data-modal-target="deleteModal-{{ $blogs->id }}"
                  data-modal-toggle="deleteModal-{{ $blogs->id }}"
  ...
  <!-- Delete modal -->
  <div id="deleteModal-{{ $blogs->id }}" tabindex="-1" aria-hidden="true"
      ...
      <div class="relative p-4 w-full max-w-md max-h-full">
          ...
          <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
              ...
              <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want
                  to delete article {{ $blogs->title }}?</p>
              <div class="flex justify-center items-center space-x-4">
                  <button data-modal-toggle="deleteModal-{{ $blogs->id }}" type="button"
                      class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No,
                      cancel</button>
                  <form action="/dashboard/{{ $blogs->slug }}" method="POST">
                      @csrf
                      @method('DELETE')
                      <button type="submit"
